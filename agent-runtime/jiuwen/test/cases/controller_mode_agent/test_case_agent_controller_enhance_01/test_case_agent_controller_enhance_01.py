...
__all__ = [
    "TestCaseControllerModeAgentEnhance01",
]
...
import pytest
from jiuwen.test.cases.workflow_node.test_case_agent_controller_enhance_01.test_intent_detection_workflow import (
    create_intent_detection_workflow,
)
from jiuwen.test.cases.workflow_node.test_case_agent_controller_enhance_01.test_rongejie_workflow import (
    create_rongEjie_workflow,
)
from jiuwen.test.cases.workflow_node.test_case_agent_controller_enhance_01.test_smart_outbound_workflow import (
    create_smart_outbound_workflow,
)
...
ZHIHUIWAIHUO_WF_ID = "5ffdcfdc-0afe-4e08-b8e8-c6206c01115e"
...
def local_ir_path(tmp_path_factory, monkeypatch):
    ...
    tmp_path = tmp_path_factory.mktemp("test_case_agent_controller_enhance_01")
...
# 测试用例
# ===================================================================
class TestCaseControllerModeAgentEnhance01:
    """
    测试场景：意图识别工作流
    """

    CONVERSATION_ID = "conv_enhance_01"
    ...
    def test_1(self, local_ir_path):
        ...
        # 意图识别工作流 mock 事件：返回 intent_id=0，触发 LLM 回退
        intent_wf_events = [
            _build_workflow_start_stream_data(workflow_id=INTENT_WF_ID),
            _build_partial_stream_data(
                answer="", node_id="node_end", node_name="结束", node_type="jiuwen.end"
            ),
            _build_partial_stream_data(
                answer="0", node_id="node_end", node_name="结束", node_type="jiuwen.end"
            ),
            _build_partial_stream_data(
                answer="", node_id="node_end", node_name="结束", node_type="jiuwen.end"
            ),
            _build_message_end_stream_data(
                ...
                outputs={"user_fields": {"intent_id": 0}},
            ),
            _build_workflow_end_stream_data(
                answer="0", node_id="node_end", node_name="结束", node_type="jiuwen.end"
            ),
            _build_end_stream_data(answer="0", user_fields={"intent_id": 0}),
        ]

        # 智能外呼工作流 mock 事件：产品介绍 + 结束节点
        zhihuiwaihuo_wf_events = [
            _build_workflow_start_stream_data(workflow_id=ZHIHUIWAIHUO_WF_ID),
            _build_partial_stream_data(
                answer="本次来电是邀请您体验我行闲钱管理服务'大天盈'的。"
                "收益每日计算，1分起购，赎回最快实时到账，让您的闲钱不闲置，"
                "详情给您发条短信，您有空的时候了解下，可以吗",
                node_id="node_1743424323024",
                node_name="产品介绍",
            ...
            _build_end_stream_data(answer="【理财】工作流执行结束"),
        ]

        workflow_runtime = _RecordingWorkflowRuntime(
            {
                INTENT_WF_ID: [intent_wf_events],
                ZHIHUIWAIHUO_WF_ID: [zhihuiwaihuo_wf_events],
            }
        )
        if os.getenv("USE_OPENJIUWEN_WORKFLOW", "false") != "true":
        ...
        assert "task_start" in event_types, "新任务场景应包含 task_start 事件"

        # CP4: workflow_start 含 ZHIHUIWAIHUO_WF_ID（智能外呼工作流被新启动）
        wf_start_events = [e for e in events if e.get("event") == "workflow_start"]
        wf_start_ids = [
            e.get("data", {}).get("workflow_id", "") for e in wf_start_events
        ]
        assert any(ZHIHUIWAIHUO_WF_ID in wid for wid in wf_start_ids), (
            f"workflow_start 应含 {ZHIHUIWAIHUO_WF_ID}，实际: {wf_start_ids}"
        )

        # CP5: message_end 含产品介绍关键文本
        message_end_answers = [
            e.get("data", {}).get("answer", "")
            for e in events
            if e.get("event") == "message_end"
        ]
        assert any("本次来电" in a for a in message_end_answers), (
            f"message_end 应含产品介绍文本，实际: {message_end_answers}"
        )

        # CP6: message_end 含工作流结束文本
        assert any("【理财】工作流执行结束" in a for a in message_end_answers), (
            f"message_end 应含结束文本，实际: {message_end_answers}"
        )

        # CP7: workflow_end 含 ZHIHUIWAIHUO_WF_ID
        wf_end_events = [e for e in events if e.get("event") == "workflow_end"]
        wf_end_ids = [e.get("data", {}).get("workflow_id", "") for e in wf_end_events]
        assert any(ZHIHUIWAIHUO_WF_ID in wid for wid in wf_end_ids), (
            f"workflow_end 应含 {ZHIHUIWAIHUO_WF_ID}，实际: {wf_end_ids}"
        )

        # CP8: done 含工作流结束文本
        done_events = [e for e in events if e.get("event") == "done"]
        assert any(
            "【理财】工作流执行结束" in e.get("data", {}).get("answer", "")
            for e in done_events
        ), f"done 事件应含结束文本，实际: {[e.get('data') for e in done_events]}"

        # CP9: task_end
        assert "task_end" in event_types, "应包含 task_end 事件"

        # CP10: LLM 被调用（意图识别工作流返回空，回退到 LLM）
        assert len(model_runtime.calls) > 0, "LLM 应被调用至少一次"

        # CP11: 智能外呼工作流被调用
        if os.getenv("USE_OPENJIUWEN_WORKFLOW", "false") != "true":
            called_wf_ids = [c["workflow_id"] for c in workflow_runtime.calls]
            assert any(ZHIHUIWAIHUO_WF_ID in wid for wid in called_wf_ids), (
                f"智能外呼工作流应被调用，实际调用: {called_wf_ids}"
            )
