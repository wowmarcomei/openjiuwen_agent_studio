...
"""
Redis bridge: full replica of agent_runtime.common.redis_manager.

Provides identical classes and functions:
  - RedisClientManager (singleton, supports single/cluster/sentinel)
  - get_redis_client()
  - ExecutionIdStore
  - QuestionerTraceStore
"""
...
from agent_builder.adapter.config_bridge import RedisMode, RedisSettings, settings
...
class RedisClientManager:
    ...
    def init(self, redis_settings: Optional[RedisSettings] = None) -> None:
        """初始化 Redis 客户端（同步调用，实际创建在异步上下文中完成）。

        Args:
            redis_settings: Redis 配置。如为 None，从全局 settings 读取。
        """
