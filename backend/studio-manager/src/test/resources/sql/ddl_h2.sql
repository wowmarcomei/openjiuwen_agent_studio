...
CREATE TABLE IF NOT EXISTS t_common_license_record  (
    ...
    PRIMARY KEY (id) ,
    UNIQUE INDEX t_common_license_record_id_uindex(id ASC) 
    ) COMMENT 'license用量记录表';
...
CREATE TABLE IF NOT EXISTS t_common_reource_inst  (
    ...
    region_id varchar(32)   NOT NULL COMMENT '下单时所在的project_id',
    project_id varchar(64)   NOT NULL COMMENT '资源状态，枚举范围active/inactive/deleted/cleaned',
    ...
    UNIQUE INDEX t_common_reource_inst_id_uindex(resource_id ASC) 
    ) COMMENT '订购实例表';
...
(
    ...
    KEY idx_t_mapping_resource_version (resource_version) ,
    KEY idx_t_mapping_app_version (app_version)
    )
    DEFAULT CHARSET = utf8mb4
...
CREATE TABLE IF NOT EXISTS t_analytics_event (
    ...
    INDEX event_time(event_time ASC) ,
    INDEX event_date(event_date ASC) ,
...
(
    id            varchar(64)  not null comment '工作空间成员表ID',
...
(
    id VARCHAR(64)  NOT NULL COMMENT '函数id' PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS t_task (
    ...
    INDEX idx_status_time(status ASC, create_time ASC) ,
    INDEX idx_finish_time(finish_time ASC) ,
...
(
    ...
    project_id VARCHAR(64)  NOT NULL COMMENT '当前企业项目ID',
    tenant_id VARCHAR(64) NOT NULL COMMENT '租户ID',
    creator_id VARCHAR(128)  NOT NULL COMMENT '创建用户ID',
...
CREATE TABLE IF NOT EXISTS t_ops_tenant_config (
    domain_id varchar(255)  NOT NULL COMMENT '租户id',
    project_id varchar(255)  DEFAULT NULL COMMENT '项目id',
    log_group_id varchar(255)  DEFAULT NULL COMMENT '日志组id',
    log_stream_id varchar(255)  DEFAULT NULL COMMENT '日志流id',
    PRIMARY KEY (domain_id)
) ;
...
(
    ...
    PRIMARY KEY (history_id) ,
    KEY                     idx_t_history_mapping_app_id (app_id) ,
    KEY                     idx_t_history_mapping_resource_id (resource_id) ,
    KEY                     idx_t_history_mapping_resource_version (resource_version) ,
    KEY                     idx_t_history_mapping_app_version (app_version)
) COMMENT='已删除资源的关系映射表';
...
CREATE TABLE IF NOT EXISTS t_model_service (
    ...
    MODEL_PRIORITY int DEFAULT NULL COMMENT '模型优先级',
...
CREATE TABLE IF NOT EXISTS t_model_service_backup (
    ...
    KEY MSB_PROVIDER_ID_INDEX (PROVIDER_ID)
) ;
...
(
    ...
    method           varchar(36)    NOT NULL COMMENT '选择的评估方法',
    task_id          varchar(36)    NOT NULL COMMENT '任务工程id',
    ...
    workspace_id     varchar(64) NULL COMMENT '工作空间ID',
    PRIMARY KEY (id) ,
    INDEX              pe_evaluation_task_id(task_id ASC)
);

CREATE TABLE IF NOT EXISTS t_pe_evaluation_result
(
    id                 varchar(36)  NOT NULL COMMENT '评估结果id',
    evaluation_task_id varchar(36)  NOT NULL COMMENT '评估任务id',
    prompt_id          varchar(36)  NOT NULL COMMENT '候选模板id',
    test_num           int                                                          NOT NULL COMMENT '测试用例行号',
    generate_result    text  NOT NULL COMMENT '结果',
...
(
    ...
    name        varchar(255)  NULL DEFAULT NULL COMMENT '模板名称',
    content     TEXT  NULL DEFAULT NULL COMMENT '模板内容',
    source      varchar(36)  NULL DEFAULT NULL COMMENT '模板来源',
    variables   TEXT  NULL DEFAULT NULL COMMENT '变量',
    pt_type     varchar(36)  NULL DEFAULT NULL COMMENT '模板类型',
    ...
    is_share    TINYINT(1) NOT NULL DEFAULT 0 COMMENT '是否已共享（0=否，1=是)',
    PRIMARY KEY (id) ,
    INDEX          key_name(name ASC) ,
    INDEX          key_project_id(project_id ASC) ,
...
CREATE TABLE IF NOT EXISTS t_history_prompt (
    ...
    source      VARCHAR(36)  NULL DEFAULT NULL COMMENT '模板来源',
    variables   TEXT  NULL COMMENT '变量',
    pt_type     VARCHAR(36)  NULL DEFAULT NULL COMMENT '模板类型',
    project_id  VARCHAR(36)  NULL DEFAULT NULL COMMENT '项目id',
    industry_id VARCHAR(36)  NULL DEFAULT NULL COMMENT '行业id',
    description VARCHAR(255)  NULL DEFAULT NULL COMMENT '描述',
    creator     VARCHAR(36)  NOT NULL COMMENT '创建人',
    updater     VARCHAR(36)  NULL DEFAULT NULL COMMENT '修改人',
    created_on  TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
...
(
    id           varchar(36)  NOT NULL COMMENT 'UUID',
    question     varchar(4000)  NULL DEFAULT NULL COMMENT '问题',
    answer       text  NULL COMMENT '答案',
    model        varchar(255)  NULL DEFAULT NULL COMMENT '模型',
...
(
    id             varchar(36)    NOT NULL COMMENT '唯一标识一个变量',
    key            varchar(36)          NOT NULL COMMENT '变量key',
    name           varchar(255)  NOT NULL COMMENT '参数名称',
    pe_task_id     varchar(36)    NOT NULL COMMENT '工程任务id',
    relation_count tinyint                                                       NOT NULL COMMENT '关联的候选模板数',
    create_time    bigint                                                        NOT NULL,
    workspace_id   varchar(64) NULL COMMENT '工作空间ID',
    PRIMARY KEY (id) ,
    KEY              idx_task (pe_task_id) ,
    KEY              UNIQUE_VARIABLE (key,pe_task_id) ,
...
(
    task_id     varchar(36)  NOT NULL COMMENT '工程任务id',
    tag_id      varchar(36)  NOT NULL COMMENT '标签id',
    workspace_id varchar(64) NULL COMMENT '工作空间ID',
    INDEX          idx_tmptt_task(task_id ASC) ,
    INDEX          idx_tmptt_tag(tag_id ASC) ,
...
(
    ...
    tag_id      varchar(36)  NOT NULL COMMENT '标签id',
    workspace_id varchar(64) NULL COMMENT '工作空间ID',
    INDEX          key_template_id(template_id ASC) ,
    INDEX          key_tag(tag_id ASC) ,
...
(
    ...
    category                VARCHAR(36)  NULL COMMENT '行业分类ID，关联 t_pe_industry.id (UUID)'
);
...
(
    ...
    trace_id                  varchar(64)       null COMMENT '溯源ID',
    visibility                VARCHAR(32) DEFAULT 'workspace' COMMENT '可见范围',
...
CREATE TABLE IF NOT EXISTS t_tool (
    ...
    icon_name         VARCHAR(64)   NULL COMMENT 'icon图标名称',
    visibility        VARCHAR(32)   NOT NULL DEFAULT 'project' COMMENT '可见范围：user,project(默认),workspace,domain,global',
    request_info      CLOB          NULL COMMENT '工具请求信息',
    auth_info         CLOB          NULL COMMENT '工具鉴权信息',
    input_schema      CLOB          NULL COMMENT '工具入参的JsonSchema',
    output_schema     CLOB          NULL COMMENT '工具出参的JsonSchema',
    intf_type         CLOB          NULL COMMENT '接口类型，取值：blocking，streaming，默认blocking',
    ...
    customize_node    TINYINT(1)       NULL COMMENT '是否是自定义节点，1表示是自定义节点,0或空表示不是',
    published         TINYINT       NULL COMMENT '是否是自定义节点，1表示是自定义节点,0或空表示不是',
    call_mode         VARCHAR(16)   DEFAULT 'api' COMMENT '插件执行类型。api：api类型的插件；functiongraph：函数类型的插件；如果为空，则默认是api;',
    domain_id         VARCHAR(64)   NULL COMMENT '租户id',
    is_input_list     CLOB          NULL COMMENT '流式封装',
    is_output_list    CLOB          NULL COMMENT '流式封装',
    auth_required     TINYINT(1)       DEFAULT 0 COMMENT '插件是否需要鉴权',
    workspace_id      VARCHAR(64)   NULL COMMENT '工作空间ID',
    trace_id          VARCHAR(64)   NULL COMMENT '溯源ID',
    is_free           TINYINT       DEFAULT 0 COMMENT '是否是免费插件，0:未知，1:免费，2:付费',
    label             VARCHAR(32)   NULL DEFAULT 'normal' COMMENT '插件类型，用于区分是否用在deepsearch',
    is_share          TINYINT(1)       NOT NULL DEFAULT 0 COMMENT '是否已共享（0=否，1=是)',
    category          VARCHAR(36)   NULL COMMENT '分类',
    PRIMARY KEY (tool_id)
    );
...
) COMMENT='tool和function多对多关系映射表';
CREATE TABLE IF NOT EXISTS t_kb_connection_router  (
                                           id varchar(100)  NOT NULL COMMENT 'id',
                                           tenant_type varchar(100)  NOT NULL COMMENT '租户类型',
                                           domain_id varchar(100)  NOT NULL COMMENT '租户id',
                                           knowledge_base_connection_id varchar(100)  NOT NULL COMMENT '知识库连接id',
                                           PRIMARY KEY (id) 
) ;

CREATE TABLE IF NOT EXISTS  t_knowledge_base  (
     id varchar(64)  NOT NULL COMMENT '知识库ID',
     NAME varchar(1000)  NULL DEFAULT NULL COMMENT '知识库名称',
     type varchar(50)  NOT NULL COMMENT '知识库类型，internal-默认，external-第三方',
     repo_type varchar(255)  NULL DEFAULT NULL COMMENT '知识库存储类型，exclusive独享，share共享',
     status varchar(10)  NOT NULL DEFAULT 'OPEN' COMMENT '启用，停用',
     icon mediumtext  NULL COMMENT '图标',
     description varchar(1000)  NULL DEFAULT NULL COMMENT '知识库描述',
     knowledge_base_connection_id varchar(100)  NULL DEFAULT NULL COMMENT '来源第三方知识库连接ID',
     external_id varchar(100)  NULL DEFAULT NULL COMMENT '第三方知识库的外部ID',
     ...
     domain_id varchar(64)  NOT NULL COMMENT '创建租户（租户id）',
     domain_name varchar(64)  NOT NULL COMMENT '创建租户（租户名）',
     created_user_id varchar(64)  NOT NULL COMMENT '创建人（用户ID）',
     created_user_name varchar(64)  NOT NULL COMMENT '创建人（用户名）',
     last_update_user_id varchar(64)  NOT NULL COMMENT '更新人（用户id）',
     last_update_user_name varchar(64)  NOT NULL COMMENT '更新人（用户名）',
     copy_source_id varchar(100)  NULL DEFAULT NULL COMMENT '复制来源的知识库ID，知识库是从其他空间复制而来时有效',
     workspace_id                VARCHAR(64)   NULL     COMMENT '工作空间ID',
     PRIMARY KEY (id) ,
     INDEX idx_domain_type(domain_id ASC, type ASC) ,
     INDEX idx_kb_connection_id(knowledge_base_connection_id ASC)
) ;

CREATE TABLE IF NOT EXISTS t_knowledge_base_config  (
                                            id varchar(64)  NOT NULL COMMENT '主键',
                                            config_type varchar(64)  NOT NULL COMMENT '配置类型',
                                            config_item varchar(64)  NOT NULL COMMENT '需要配置的具体条目',
                                            value mediumtext  NOT NULL COMMENT '值',
                                            description varchar(255)  NOT NULL COMMENT '描述',
                                            domain_id varchar(64)  NOT NULL COMMENT '租户id',
                                            PRIMARY KEY (id, domain_id) ,
                                            UNIQUE INDEX t_knowledge_base_config_unique(config_type ASC, config_item ASC, domain_id ASC) 
) ;

CREATE TABLE IF NOT EXISTS t_knowledge_base_connection  (
                                                id varchar(64)  NOT NULL COMMENT '第三方知识库连接id',
                                                connector_id varchar(64)  NOT NULL COMMENT '使用的连接器id',
                                                name varchar(64)  NOT NULL COMMENT '连接名称',
                                                description varchar(1000)  NULL DEFAULT NULL COMMENT '描述',
                                                icon mediumtext  NOT NULL COMMENT '连接器图标',
                                                used_abilities mediumtext  NULL COMMENT '开启能力集合, 使用JSON结构存储',
                                                params mediumtext  NULL COMMENT '连接信息参数',
                                                ...
                                                krb5_file text  NULL COMMENT 'Kerberos二进制密钥文件',
                                                keytab_file text  NULL COMMENT 'Kerberos配置文件',
                                                status varchar(20)  NULL DEFAULT NULL COMMENT '状态，OPEN-启用，CLOSE-停用',
                                                domain_id varchar(64)  NOT NULL COMMENT '租户id',
                                                domain_name varchar(64)  NOT NULL COMMENT '租户名',
                                                create_user_id varchar(64)  NOT NULL COMMENT '创建人ID',
                                                create_user_name varchar(64)  NOT NULL COMMENT '创建人名称',
                                                ...
                                                cluster_type varchar(255) NULL COMMENT '集群类型',
                                                max_tenant_count int NULL COMMENT '支持的最大租户数量',
                                                remaining_tenant_quota int NULL COMMENT '剩余可用租户数量',
                                                project_id varchar(64)  NULL DEFAULT NULL COMMENT '项目ID',
                                                PRIMARY KEY (id) 
) ;

CREATE TABLE IF NOT EXISTS t_knowledge_base_connector  (
                                               id varchar(64)  NOT NULL COMMENT '连接器ID',
                                               name varchar(100)  NOT NULL COMMENT '连接器名称',
                                               icon mediumtext  NOT NULL COMMENT '连接器图标',
                                               description varchar(1000)  NULL DEFAULT NULL COMMENT '连接器描述',
                                               deploy_mode varchar(64)  NULL DEFAULT NULL COMMENT '适用的部署场景，比如HC、HCS等，适用多个场景时使用,分割',
                                               type varchar(64)  NULL DEFAULT NULL COMMENT '知识源接入类型',
                                               param_definition mediumtext  NULL COMMENT '参数定义,JSON结构',
                                               help_text mediumtext  NULL COMMENT '帮助说明',
                                               domain_id varchar(64)  NOT NULL COMMENT '租户id',
                                               domain_name varchar(64)  NOT NULL COMMENT '租户名',
                                               create_user_id varchar(64)  NOT NULL COMMENT '创建人ID',
                                               create_user_name varchar(64)  NOT NULL COMMENT '创建人名称',
...
CREATE TABLE IF NOT EXISTS t_knowledge_base_connector_ability  (
                                                       id varchar(50)  NOT NULL COMMENT '连接器能力ID',
                                                       CODE varchar(50)  NOT NULL COMMENT '连接器能力编码',
                                                       NAME varchar(100)  NOT NULL COMMENT '连接器能力名称',
                                                       description varchar(1000)  NULL DEFAULT NULL COMMENT '连接器能力描述',
                                                       connector_id varchar(20)  NOT NULL COMMENT '归属的连接器ID',
                                                       sub_ability mediumtext  NULL COMMENT '子能力信息，使用JSON结构存储',
                                                       PRIMARY KEY (id) 
) ;

CREATE TABLE IF NOT EXISTS t_knowledge_base_model  (
                                           id varchar(64)  NOT NULL COMMENT 'id',
                                           type varchar(64)  NULL DEFAULT NULL COMMENT '模型类型',
                                           NAME varchar(64)  NOT NULL COMMENT '注册到koosearch的名称',
                                           model_name varchar(64)  NULL DEFAULT NULL COMMENT '模型名称,实际被调用的模型名称',
                                           knowledge_base_connection_id varchar(64)  NULL DEFAULT NULL COMMENT '知识库连接id',
...
CREATE TABLE IF NOT EXISTS t_knowledge_base_obs_config  (
                                                id varchar(64)  NOT NULL COMMENT '配置ID',
                                                knowledge_base_id varchar(64)  NULL DEFAULT NULL COMMENT '知识库ID',
                                                obs_bucket_name varchar(255)  NULL DEFAULT NULL COMMENT 'obs桶名',
                                                obs_input_directory varchar(255)  NULL DEFAULT NULL COMMENT 'obs路径',
                                                task_status varchar(36)  NULL DEFAULT NULL COMMENT '任务状态',
                                                domain_id varchar(64)  NOT NULL COMMENT '租户id',
                                                domain_name varchar(64)  NOT NULL COMMENT '租户名',
                                                created_user_id varchar(64)  NOT NULL COMMENT '创建人ID',
                                                created_user_name varchar(64)  NOT NULL COMMENT '创建人名称',
                                                create_time bigint NOT NULL COMMENT '创建日期',
                                                update_user_id varchar(64)  NOT NULL COMMENT '更新人ID',
                                                update_user_name varchar(64)  NOT NULL COMMENT '更新人名称',
                                                update_time bigint NOT NULL COMMENT '更新日期',
                                                project_id varchar(64)  NULL DEFAULT NULL COMMENT '项目ID',
                                                PRIMARY KEY (id) ,
                                                UNIQUE INDEX knowledge_obs_config_idx(knowledge_base_id ASC) 
) ;

CREATE TABLE IF NOT EXISTS t_knowledge_base_obs_execute_record  (
                                                        id varchar(64)  NOT NULL COMMENT '调度记录id',
                                                        obs_config_id varchar(64)  NULL DEFAULT NULL COMMENT '知识库ID',
                                                        ...
                                                        log_detail longtext  NULL COMMENT '日志信息',
                                                        domain_id varchar(64)  NOT NULL COMMENT '租户id',
                                                        domain_name varchar(64)  NOT NULL COMMENT '租户名',
                                                        created_user_id varchar(64)  NOT NULL COMMENT '创建人ID',
                                                        created_user_name varchar(64)  NOT NULL COMMENT '创建人名称',
                                                        ...
                                                        project_id varchar(64)  NULL DEFAULT NULL COMMENT '项目ID',
                                                        PRIMARY KEY (id) ,
                                                        INDEX obs_config_execute_idx(obs_config_id ASC) 
) ;

CREATE TABLE IF NOT EXISTS t_knowledge_clean_task_info  (
                                                task_id varchar(64)  NOT NULL COMMENT '任务ID',
                                                domain_id varchar(64)  NOT NULL COMMENT 'DOMAINID',
                                                STATUS varchar(10)  NULL DEFAULT 'PROCESSING' COMMENT '任务状态',
                                                create_time bigint NOT NULL COMMENT '创建时间',
                                                finish_time bigint NULL DEFAULT NULL COMMENT '任务完成时间',
                                                PRIMARY KEY (task_id) ,
                                                INDEX idx_domain_id(domain_id ASC) 
) ;

CREATE TABLE IF NOT EXISTS t_knowledge_i18n_resource  (
                                              id bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
                                              i18n_key varchar(255)  NOT NULL COMMENT '国际化Key',
                                              locale varchar(50)  NOT NULL COMMENT '语言区域 (如 zh-cn, en-us)',
                                              message text  NOT NULL COMMENT '翻译后的展示文案',
                                              PRIMARY KEY (id) ,
                                              UNIQUE INDEX uk_key_locale(i18n_key ASC, locale ASC) 
) ;

CREATE TABLE IF NOT EXISTS t_knowledge_repo  (
                                     knowledge_repo_id varchar(64)  NOT NULL COMMENT '知识库id',
                                     project_id varchar(64)  NULL DEFAULT NULL COMMENT '租户唯一标识',
                                     display_name varchar(64)  NOT NULL COMMENT '知识库名称',
                                     desc varchar(128)  NOT NULL COMMENT '知识库描述',
                                     type varchar(32)  NOT NULL COMMENT '知识库类型，internal：OP账号下的知识库，external：用户账号下的知识库',
                                     icon mediumtext  NULL COMMENT '知识库图标',
                                     icon_name varchar(64)  NULL DEFAULT NULL COMMENT 'icon图标名称',
                                     size bigint NOT NULL COMMENT '知识库中所有文件的总大小（Byte）',
                                     file_num int NOT NULL COMMENT '知识库中所有文件数量',
                                     domain_id varchar(64)  NULL DEFAULT NULL COMMENT '创建租户（租户id）',
                                     domain_name varchar(64)  NULL DEFAULT NULL COMMENT '创建租户（租户名）',
                                     creator varchar(64)  NULL DEFAULT NULL COMMENT '知识库创建者',
                                     ...
                                     INDEX idx_t_knowledge_repo_updated_on(updated_on ASC) 
) ;

CREATE TABLE IF NOT EXISTS t_knowledge_segment_rule  (
                                             id varchar(64)  NOT NULL COMMENT '规则id',
                                             project_id varchar(64)  NULL DEFAULT NULL COMMENT '租户唯一标识',
                                             domain_id varchar(64)  NULL DEFAULT NULL COMMENT '创建租户（租户id）',
                                             domain_name varchar(64)  NULL DEFAULT NULL COMMENT '创建租户（租户名）',
                                             rule mediumtext  NOT NULL COMMENT '用户定义规则',
...
CREATE TABLE IF NOT EXISTS t_knowledge_share_scope  (
                                            id varchar(64)  NOT NULL COMMENT 'id',
                                            knowledge_base_id varchar(64)  NULL DEFAULT NULL COMMENT '被共享的知识库id',
                                            workspace_id varchar(64)  NULL DEFAULT NULL COMMENT '被授权的空间id',
                                            project_id varchar(64)  NULL DEFAULT NULL COMMENT '项目id',
                                            created_user_id varchar(64)  NULL DEFAULT NULL COMMENT '创建人id',
                                            created_user_name varchar(64)  NULL DEFAULT NULL COMMENT '创建人名称',
                                            create_time bigint NULL DEFAULT NULL COMMENT '创建时间',
                                            PRIMARY KEY (id) 
) ;

CREATE TABLE IF NOT EXISTS t_knowledge_test_record  (
                                            record_id varchar(64)  NOT NULL COMMENT '测试记录id',
                                            knowledge_repo_id varchar(64)  NOT NULL COMMENT '知识库id',
                                            project_id varchar(64)  NULL DEFAULT NULL COMMENT '租户项目id',
                                            domain_id varchar(64)  NULL DEFAULT NULL COMMENT '创建租户（租户id）',
                                            domain_name varchar(64)  NULL DEFAULT NULL COMMENT '创建租户（租户名）',
                                            query varchar(4096)  NOT NULL COMMENT '测试的输入',
                                            created_on timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
                                            PRIMARY KEY (record_id) ,
                                            INDEX idx_t_knowledge_test_record_created_on(created_on ASC) 
) ;

CREATE TABLE IF NOT EXISTS t_memory_variable_value  (
                                            variable_key varchar(255)  NOT NULL COMMENT '变量的key',
                                            variable_value varchar(1000)  NULL DEFAULT NULL COMMENT '变量的值',
